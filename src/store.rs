...
#[derive(Clone)]
pub struct MemoryStore { pool: PgPool }
...
impl MemoryStore {
    ...
    pub async fn migrate(&self) -> Result<(), sqlx::migrate::MigrateError> { sqlx::migrate!().run(&self.pool).await }
    pub async fn ping(&self) -> Result<(), sqlx::Error> { sqlx::query("SELECT 1").execute(&self.pool).await.map(|_| ()) }
    ...
    pub async fn supersede(&self, old_id: Uuid, tenant_id: Uuid, input: &AppendClaim, embedding: Vector) -> Result<Claim, StoreError> {
        let mut tx = self.pool.begin().await?;
        ...
        if updated.rows_affected() == 0 { return Err(StoreError::NotFound); }
    ...
    pub async fn recall(&self, request: &RecallRequest, embedding: Vector) -> Result<Vec<RecallHit>, StoreError> {
        let lexical_weight = 1.0 - request.semantic_weight;
        Ok(sqlx::query_as::<_, RecallHit>(include_str!("../sql/recall.sql"))
            .bind(request.tenant_id).bind(&request.query).bind(embedding)
            .bind(request.semantic_weight).bind(lexical_weight).bind(request.limit)
            .fetch_all(&self.pool).await?)
    }
}

async fn insert_claim(tx: &mut Transaction<'_, Postgres>, input: &AppendClaim, embedding: Vector) -> Result<Claim, sqlx::Error> {
...
impl AppendClaim {
    fn clone_for_supersede(&self, old_id: Uuid, tenant_id: Uuid) -> Self {
        Self { tenant_id, subject: self.subject.clone(), predicate: self.predicate.clone(), object: self.object.clone(), source: self.source.clone(), confidence: self.confidence, content: self.content.clone(), embedding: self.embedding.clone(), valid_from: self.valid_from, valid_until: self.valid_until, supersedes_claim_id: Some(old_id) }
    }
}
