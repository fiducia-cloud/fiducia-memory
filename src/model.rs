...
impl AppendClaim {
    pub fn validate(&self) -> Result<Vector, &'static str> {
        if self.subject.trim().is_empty() || self.predicate.trim().is_empty() || self.content.trim().is_empty() {
...
impl RecallRequest {
    pub fn validate(&self) -> Result<Vector, &'static str> {
        if self.query.trim().is_empty() { return Err("query must be non-empty"); }
        if !(1..=100).contains(&self.limit) { return Err("limit must be between 1 and 100"); }
        if !(0.0..=1.0).contains(&self.semantic_weight) { return Err("semantic_weight must be between 0 and 1"); }
        if self.embedding.len() != EMBEDDING_DIMENSIONS { return Err("embedding must contain exactly 1536 values"); }
        Ok(Vector::from(self.embedding.clone()))
    }
}
